...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                exclude-result-prefixes="func exsl"
...
<xsl:template match="/">
  ...
  </xsl:variable><!-- <xsl:text> Whent here</xsl:text>-->
...
<xsl:template name="layout2svg">
  ...
  <svg:svg viewBox="0 0 {sum($layout/node/@width) * 2 * $hedge.scale} {$maxDepth * 2 * $hedge.scale}"  
  	width="{sum($layout/node/@width)*50}mm" depth="{$maxDepth*50}mm" preserveAspectRatio="xMidYMid meet" >
